  ...
  <div class="flex flex-col justify-start bg-white p-6">
    <div class="flex gap-4">
      @foreach ($post->categories as $category)
        <a href="{{ $category->title }}" class="pb-4 text-sm font-bold uppercase text-blue-700">{{ $category->title }}</a>
      @endforeach
    </div>

    <a href="{{ route('view', $post) }}" class="pb-4 text-3xl font-bold hover:text-gray-700">{{ $post->title }}</a>
